...
## TextViewAnimator and bitmap-font text

`TextViewAnimator` draws text that was assembled from one bitmap per character. It is
a Canvas helper, not an Android XML `TextView`.

Create the bitmap-font map and add a centered clickable label:
...
Forward drawing and touch from the host custom view:
...
Release active press animators with the host:
...
Available creation overloads include absolute `left`/`top`, optional character spacing,
and `PointF + referenceWidth + runtimeWidth` coordinate conversion. Alignment uses
`TextWriter.LineType.START`, `MIDDLE`, or `END`.

The supporting `com.ogfa.nativeviews.text` package provides:
...
font rendering and line wrapping. The SDK embeds the required middle-removal logic and
does not depend on Carrom's `BitmapUtil` or `BoardProperty`. AndroidX Core is compile-only
and remains supplied by the parent application through AppCompat.
